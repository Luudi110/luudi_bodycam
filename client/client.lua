...
RegisterNetEvent('spy-bodycam:startWatchingDashcam',function(netId)
    ...
    SetEntityVisible(cache.ped, false) -- Sæt usynlig
    SetEntityCollision(cache.ped, false, false) -- Sæt kollision
    SetEntityInvincible(cache.ped, true) -- Sæt udødelig
    NetworkSetEntityInvisibleToNetwork(cache.ped, true) -- Sæt usynlig i netværket
    ...
    if Config.DebugCamera then 
        Citizen.CreateThread(function()
            ...
            while PlyInCarCam do
                ...
                if IsDisabledControlJustReleased(0, 35) then -- A (venstre)
                    offsetX = offsetX + 0.01
                end
                if IsDisabledControlJustReleased(0, 34) then -- D (højre)
                    offsetX = offsetX - 0.01
                end
                if IsDisabledControlJustReleased(0, 44) then -- Q (frem)
                    offsetY = offsetY + 0.01
                end
                if IsDisabledControlJustReleased(0, 38) then -- E (tilbage)
                    offsetY = offsetY - 0.01
                end
                if IsDisabledControlJustReleased(0, 32) then -- W (op)
                    offsetZ = offsetZ + 0.01
                end
                if IsDisabledControlJustReleased(0, 33) then -- S (ned)
                    offsetZ = offsetZ - 0.01
                end
...
function SetPlayerNearTarget()
    while PlyInCam or PlyInCarCam do
        ...
        Wait(2500) -- Tjek distancen hvert angivne sekund
    end
end
...
function QuitBodyCam()
    ...
    SetEntityVisible(cache.ped, true) -- Sæt synlig
    SetEntityCollision(cache.ped, true, true) -- Gendan kollision
    SetEntityInvincible(cache.ped, false) -- Fjern udødelighed
    NetworkSetEntityInvisibleToNetwork(cache.ped, false) -- Gendan synlig i netværket
    FreezeEntityPosition(cache.ped, false)
