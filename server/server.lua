...
RegisterNetEvent('spy-bodycam:server:logVideoDetails', function(videoUrl,streetName)
    ...
    ---SQL UPLOAD
    MySQL.Async.execute('INSERT INTO luudi_bodycam (job, videolink, street, date, playername) VALUES (@job, @videolink, @street, @date, @playername)', {
        ['@job'] = jobKey,
        ['@videolink'] = videoUrl,
        ['@street'] = streetName,
        ['@date'] = date,
...
RegisterNetEvent('spy-bodycam:server:deleteVideoDB', function(videoUrl)
    ...
    MySQL.Async.execute('DELETE FROM luudi_bodycam WHERE videolink = @videolink', {
        ['@videolink'] = videoUrl
        ...
        if rowsChanged > 0 then
            ...
            MySQL.Async.fetchAll('SELECT * FROM luudi_bodycam WHERE job = @job ORDER BY id DESC', {
                ['@job'] = jobKey
            }, function(records)
...
-- Du kan fjerne dette for bedre optimering af scriptet.
-- Hvis køretøjet slettes eller sendes til garage, fjernes det fra listen.
Citizen.CreateThread(function()
    while true do
        ...
        Citizen.Wait(60000) -- Vent 1 minut før næste kontrol
    end
end)

-- Script versionskontrol
local localVersion = GetResourceMetadata(GetCurrentResourceName(), 'version')
local fxManifestUrl = "https://raw.githubusercontent.com/Your-Luudi/spy-bodycam/main/%5Bspy-bodycam%5D/spy-bodycam/fxmanifest.lua"
